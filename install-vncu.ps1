...
function install-VNC {
    ...
    if ($software_is_installed) {
        ...
        if ($result) {
            ...
            Write-Log -logfile $log_file -message "Find old VNC version:$($software_is_installed.DisplayVersion),$env:COMPUTERNAME,$ipv4"
    ...
    if ($null -eq $software_is_installed) {
        ...
        if ($null -ne $software_exec) {
            ...
            write-host $install_filepath
            $install_arrg = "/i $install_filepath /passive /norestart SERVERVIEWER=3 SERVICE=1 PASSWORD=""sysc0012"""
            
            write-host $install_arrg
