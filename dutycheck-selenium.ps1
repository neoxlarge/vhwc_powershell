...
function check-oe( $check_item, $branch, $url, $account, $password, $save_path, $capture_area) {
...
foreach ($key in $check_oe.keys) {
    check-oe -check_item $key -branch $check_oe[$key]['branch'] -url $check_oe[$key]['url'] -account $check_oe[$key]['account'] -password $check_oe[$key]['password'] -save_path 'd:\mis' -capture_area $check_oe[$key]['capture_area']
}

foreach ($key in $check_showjob.keys) {
    check-showjob -check_item $key -branch $check_showjob[$key]['branch'] -url $check_showjob[$key]['url'] -save_path 'd:\mis' -capture_area $check_showjob[$key]['capture_area']
}

foreach ($key in $check_cyp2001.keys) {
    check-cyp2001 -check_item $key -branch $check_cyp2001[$key]['branch'] -account $check_cyp2001[$key]['account'] -password $check_cyp2001[$key]['password'] -url_login $check_cyp2001[$key]['url_login'] -url_query $check_cyp2001[$key]['url_query'] -save_path 'd:\mis'
}