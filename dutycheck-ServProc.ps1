...
do {
    ...
    foreach ($server in $server_list ) {
        ...
        if ($ping) {
            write-debug "$($server.title): $($server.ip) 連線成功"
        }
        else {
            Write-Debug "$($server.title): $($server.ip) 連線失敗"
            # Send-LineNotifyMessage -Message "$($server.title): $($server.ip) 連線失敗" -StickerPackageId 1 -StickerId 100
            continue
        }
        ...
        if ($ping) {
            ...
            # 連線到遠端電腦並取得執行中的程式
            Write-Debug "連線到 $($server.ip) 並取得執行中的程式..."
            ...
            Write-Debug "連線到 $($server.ip) 並取得指定程式的 CPU 使用率..."
